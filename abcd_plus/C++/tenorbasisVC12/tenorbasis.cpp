...
namespace QuantLib {
	...
	std::vector<Real> AbcdTenorBasis::cadstToCoeff(const Real & c, const std::vector<Real> & adst) const {
	...
	std::vector<Real> AbcdTenorBasis::parameterConversion(std::vector<Real> coeff, std::vector<Real> guess, Real accuracy, Real min,Real max) const {
		Brent solver;
    ...
    :calibratedModel_(calibratedModel), helpers_(helpers), method_(method), 
                      endCriteria_(endCriteria),fixParameters_(fixParameters) {
		//!check on weights
		Integer sizeHelpers = helpers_.size();
		Integer sizeWeights = weights.size();

		if (sizeHelpers != sizeWeights) {
			std::vector<QuantLib::Real> w;
			w.resize(sizeHelpers);
			for (Integer i = 0; i < sizeHelpers; i++) {
				w[i] = 1.0;
			}
			weights_ = w;
		}
    ...
    Real GlobalError::operator()(Real guess) const {
		...
		Integer index = position_.size() - innerErrorNumber_;
