...
namespace QuantLib {
    ...
    class TenorBasis : public CalibratedModel {
      public:
		...
		virtual std::vector<Real> parameterConversion(std::vector<Real> coeff, std::vector<Real> guess,Real accuracy, Real min, Real max )const = 0;
      protected:
    ...
    class AbcdTenorBasis : public TenorBasis {
      public:
		...
		std::vector<Real> parameterConversion(std::vector<Real> coeff, std::vector<Real> guess, Real accuracy = 0.0000001, Real min = 0.0001, Real max = 3.0)const;
		Real parameterConversionError(const Real & c , const Real & a, const Real & d, const Real & sMax, const Real & tMax) const;
		virtual std::vector<Real> cadstToCoeff(const Real & c, const std::vector<Real> & adst) const;
      protected:
        //! \name TenorBasis Interface
        //@{
    ...
    class PolynomialTenorBasis : public TenorBasis {
      public:
        ...
        const std::vector<Real>& coefficients() const;
        const std::vector<Real>& instCoefficients() const;
		//trivial implementation, just for satisfying the pure virtual constraint
		std::vector<Real> parameterConversion(std::vector<Real> coeff, std::vector<Real> guess, Real accuracy = 0.0000001, Real min = 0.0001, Real max = 3.0)const{ return coeff;};
        //@}
      protected:
        //! \name TenorBasis Interface
        //@{
    ...
    class GlobalError {
        public:
            GlobalError(
                const std::vector<boost::shared_ptr<GlobalHelper>> & helpers,
                const std::vector<Integer>& position,
                const Integer& innerErrorNumber,
                const Real& accuracy,
                const Real& min,
                const Real& max);
        ...
        private:
            std::vector<boost::shared_ptr<GlobalHelper>> helpers_;
            Brent firstSolver_;
            mutable Integer innerErrorNumber_;
            std::vector<Integer> position_;
            Real accuracy_;
            Real min_;
            Real max_;
